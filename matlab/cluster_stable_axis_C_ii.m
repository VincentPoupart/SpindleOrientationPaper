 ...
 Angle_projected_Stable_Axis_projected_Spindle = nan(length(Celloutput),1);
...
for i = 1:1:length(Celloutput)
    ...
    if length(y(~isnan(y))) >= 3
              ...
              if nanmin(Celloutput(i).meas(ioa-4:ioa,31))>125 
              if nanmax(Celloutput(i).meas(ioa-4:ioa,31))<200     
    ...
    Angle_projected_Stable_Axis_projected_Spindle(i,1) = nanmean(Celloutput(i).meas(ioa:ioa+2,59));
...
A4=(Angle_projected_Stable_Axis_projected_Spindle(~isnan(Angle_projected_Stable_Axis_projected_Spindle)));
