 Angle_Stable_Axis_A_points = nan(length(Celloutput),1);
 Angle_Un_Stable_Axis_A_points = nan(length(Celloutput),1);
 Angle_Rachis_Stable_Axis_A_points = nan(length(Celloutput),1);
 Angle_projected_Stable_Axis_A_projected_Spindle = nan(length(Celloutput),1);
 Angle_projected_Un_Stable_Axis_A_projected_Spindle = nan(length(Celloutput),1);
 Angle_spindle = nan(length(Celloutput),1);
 Range_short_axis = nan(length(Celloutput),1);
...
for i = 1:1:length(Celloutput)
    Celloutput(i).stable_axis_A = [NaN NaN NaN];
...
%     if length(x(~isnan(x))) >= 5
              ...
              MeanShortAxis = nanmean(Celloutput(i).meas(ioa-4:ioa,21:23));
              points = Celloutput(i).meas(ioa-4:ioa,21:23);
              for v = 1:1:length(points)
                  vector = points(v,:);
                  angle = (acos(dot(MeanShortAxis,vector)/(norm(MeanShortAxis)*norm(vector))))*180/pi();
                          if angle > 90
                           points(v,:) =  vector * -1;
                          end
              end
              ...
              if sum(fooot)>1
              Celloutput(i).stable_axis_A = nanmean(points(fooot,1:3));
              Celloutput(i).sumfooot = sum(fooot);
              else
              Celloutput(i).stable_axis_A = points(fooot,1:3);
              end
              ...
              axis = Celloutput(i).stable_axis_A;
 
              for j = 1:1:length(Celloutput(i).meas(:,1))
              
              axis2= Celloutput(i).meas(j,21:23);    
    ...
    Angle_Stable_Axis_A_points(i,1) = nanmean(Celloutput(i).meas(ioa:ioa+2,56));
    Angle_Un_Stable_Axis_A_points(i,1) = nanmean(Celloutput(i).meas(ioa:ioa+2,57));
    Angle_Rachis_Stable_Axis_A_points(i,1) = nanmean(Celloutput(i).meas(ioa:ioa+2,58));
    Angle_projected_Stable_Axis_A_projected_Spindle(i,1) = nanmean(Celloutput(i).meas(ioa:ioa+2,59));
    Angle_projected_Un_Stable_Axis_A_projected_Spindle(i,1) = nanmean(Celloutput(i).meas(ioa:ioa+2,63));
    ...
    for d = 1:1:length(ComparisonAllVector)
        for e = 1:1:length(ComparisonAllVector)
          axisI = Celloutput(i).meas(ioa+1-d,21:23);
          axisII = Celloutput(i).meas(ioa+1-e,21:23);
    ...
    Range_short_axis(i,1) = max(ComparisonAllVector(:));
    
    if isnan(Angle_spindle(i,1))
        Range_short_axis(i,1) = NaN;
    end
...
% end
% end 
B=(Angle_Stable_Axis_A_points(~isnan(Angle_Stable_Axis_A_points)));
B2=(Angle_Un_Stable_Axis_A_points(~isnan(Angle_Un_Stable_Axis_A_points)));
B3=(Angle_Rachis_Stable_Axis_A_points(~isnan(Angle_Rachis_Stable_Axis_A_points)));
B4=(Angle_projected_Stable_Axis_A_projected_Spindle(~isnan(Angle_projected_Stable_Axis_A_projected_Spindle)));
B5=(Angle_projected_Un_Stable_Axis_A_projected_Spindle(~isnan(Angle_projected_Un_Stable_Axis_A_projected_Spindle)));
B6=(Angle_spindle(~isnan(Angle_spindle))); 
% Prolate_Index=(Prolate_Index(~isnan(Prolate_Index))); 
% Range_short_axis = (Range_short_axis(~isnan(Range_short_axis))); 
