...
function FetchImarisStats(filelist::Vector{String})
    ...
    Cell_Shape_df = leftjoin(Cell_Ellipsoid_Axis_C_df, Cell_Volume_df; on=[:Gonad, :Time, :TrackID])
    ...
    for i in 1:nrow(CellShape_df_final)
        ...
        if CellShape_df_final[i, :MeanCellVolume] <= 175
            CellSize[i] = "Small"
        else
            CellSize[i] = "Big"
        end
